...
addExample("Load mp3 soundfont file", function() {
  load("http://gleitz.github.io/midi-js-soundfonts/FluidR3_GM/marimba-mp3.js")
    .then(playNotes("C4 D4 E4 F4 G4 A4 B4 C5"))
    .catch(err);
});
...
addExample("Load drum machine", function() {
  fetch("https://oramics.github.io/sampled/DM/CR-78/sampled.instrument.json")
    .then(x => x.json())
    .then(meta => load(meta.files, { from: meta.samples }))
    .then(function(samples) {
      playArray(Object.keys(samples).map(name => samples[name]))
    })
    .catch(err);
});
...
function playObject(obj) {
  playArray(
    Object.keys(obj).map(function(n) {
      return obj[n];
    })
  );
}
